...
while 1:
    ...
    crop = frame[height - 260:height - 240, int(rect[0]):int(rect[1])].copy()
    ...
    cv2.rectangle(frame, (int(rect[0]), height - 250), (int(rect[1]), height - 230), (255, 0, 0), 2)
    robot.set_frame(frame, 20)

    coordinate_abs = rect[0] + coordinate_rel
    print(coordinate_rel, coordinate_abs)

    rect[0] = robot.constrain(coordinate_abs - 30, 110, 550)
    rect[1] = robot.constrain(coordinate_abs + 30, int(rect[0]) + 1, 550)
